...
def download_master_file():  
    ...
    if not url:  
        raise ValueError("MASTER_FILE_URL is not set in .env")  
...
@app.route('/')  
def home():  
    return 'Welcome to my API bot for creating file banners'  
...
def update_directus():  
    try:  
        ...
        print("Generated JSON Data:", json.dumps(data, indent=2, ensure_ascii=False))  

        # Send POST request to AWS API Gateway  
        aws_api_url = os.getenv('AWS_API_URL')  
        if not aws_api_url:  
            return jsonify({'error': 'AWS_API_URL is not set in .env'}), 500  
        ...
        try:  
            ...
            print(f"Error sending data to AWS API Gateway: {e}")  
            return jsonify({'error': str(e)}), 500  

        # Debug: Print the response from AWS API Gateway  
        print("Response from AWS API Gateway:", response.status_code, response.text)  
        ...
        print(f"Error in update_directus: {str(e)}")  
        return jsonify({'error': str(e)}), 500  
